...
class MyPIIFilter(Validator):
    PII_REGEX = r"\b(?:\d{3}[-.\s]??\d{2}[-.\s]??\d{4}|\d{9}|\d{4}[-.\s]??\d{4}[-.\s]??\d{4}|\d{3}[-.\s]??\d{3}[-.\s]??\d{3}|\(\d{3}\)\s*\d{3}[-.\s]??\d{4}|\d{3}[-.\s]??\d{2}[-.\s]??\d{4})\b"  # noqa

    def validate(self, value: Any, metadata: Dict) -> ValidationResult:
        if re.search(self.PII_REGEX, value):
            return FailResult(
                error_message="The text contains Personally Identifiable Information (PII)."  # noqa
            )
        return PassResult()
...
class TextAnalysis(BaseModel):
    text: str = Field(
        description="Text to analyze for Personally Identifiable Information (PII).",  # noqa
    )
...
guard = (
    gd.Guard()
    .use(MyPIIFilter, on_fail="exception")
    .from_pydantic(output_class=TextAnalysis, prompt=prompt)
)

try:
    raw_llm_response, validated_response, *rest = guard(
        openai.completions.create,
        prompt_params={"text": "John Doe's SSN is XXX-XX-XXXX."},
